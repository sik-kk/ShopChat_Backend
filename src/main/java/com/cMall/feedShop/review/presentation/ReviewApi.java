...
public interface ReviewApi {
    ...
    ResponseEntity<ApiResponse<ReviewDetailResponse>> updateReview(
    ...
    ResponseEntity<ApiResponse<ReviewDetailResponse>> getReview(
            @PathVariable @Min(1) @Parameter(description = "리뷰 ID") Long reviewId
    );

    @Operation(summary = "상품별 리뷰 목록 조회", description = "특정 상품의 리뷰 목록을 페이징하여 조회합니다.")
    ResponseEntity<ApiResponse<ProductReviewSummaryResponse>> getReviewsByProductId(
    ...
    ResponseEntity<ApiResponse<Page<ReviewDetailResponse>>> getReviewsByUserId(
    ...
    ResponseEntity<ApiResponse<Page<ReviewSummaryResponse>>> searchReviews(
